...
.\.venv\Scripts\python.exe -c "import py_compile, pathlib; [py_compile.compile(str(p), doraise=True) for p in pathlib.Path('clash_speed').glob('*.py')]"
```

## 代码结构

- `clash_speed/`：核心包，包含默认值、数据模型、Clash API 客户端、测速实现、并发 runner、结果转换和 CLI 输出。
- `clash_speed_test.py`：兼容旧命令和旧导入的薄入口，继续支持 `python clash_speed_test.py ...`。
- `app.py`：Streamlit Web UI，只负责页面、会话状态、进度和图表展示，复用 `clash_speed` 中的业务逻辑。
